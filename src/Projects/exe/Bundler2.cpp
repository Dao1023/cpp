...
static int num = 0;
//在这里定义释放后的exe名称
static string EXE = "7z.exe";
//在这里定义释放后的附件名称
static string Filename = "tabulary.pdf";
//在这里定义释放路径具体位置
CHAR czTempPath[MAX_PATH] = "C:\\Users\\Dao\\Projects\\Programming\\C++\\exe\\resources\\";

//定义 EnumTypesFunc() 函数，该函数用于处理枚举到的资源类型。
void EnumTypesFunc(HMODULE hModule, LPWSTR lpType, LONG_PTR lParam) {
    string Name = "";
    CHAR PathFileName[MAX_PATH] = { 0 };
    CHAR FileName[MAX_PATH] = { 0 };

    // 使用 wcstombs 或类似函数转换字符集
    char a[10];
    wcstombs(a, lpType, 10);
